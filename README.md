...
- The callback context is the authoritative request interface: use `context.serializeRoot()` and `context.serialize(element)` so serialization stays bound to that session's DOM. Top-level serializers remain compatibility helpers for the active global DOM.
...
const output = await withServerDOM(
  {},
  ({ document, library, serializeRoot }) => {
